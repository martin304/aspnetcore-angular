    ...
    public class MappingProfile:Profile
    {
        public MappingProfile()
        {
            ...
            CreateMap<VehicleResource,Vehicle>()
             ...
             var  removedFeatures= v.Features.Where(vf=>!vr.Features.Any(f=>f.Id==vf.FeatureId));
                  foreach(var f in removedFeatures){
                      v.Features.Remove(f);
                  }
                  //add new features
               var addedFeatures=   vr.Features.Where(f=>!v.Features.Any(vf=>vf.FeatureId==f.Id)).Select(f=>new VehicleFeature{FeatureId=f.Id });
